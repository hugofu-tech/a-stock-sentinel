...
- **2026-03-27 重写**: 三个爬虫全部改用akshare可靠接口替代直接爬取
  - 雪球: akshare stock_hot_tweet_xq（讨论热度排行）+ stock_news_em（个股新闻）
  - 微博: akshare stock_js_weibo_report（金十数据微博舆情评分）+ stock_news_em
  - 市值风云: 域名已迁移至wogoo.com（SPA需认证），改用akshare stock_news_main_cx（财新新闻）+ stock_news_em
  - 四个数据源全部通过健康检查（4/4），集成测试155条帖子/3只股票
...
### 2026-03-27 爬虫重写：解决WAF/反爬问题
- [x] xueqiu_crawler.py — 改用akshare接口（stock_hot_tweet_xq + stock_news_em）
  - 原因：阿里云WAF拦截所有直接API请求，返回JavaScript挑战
  - 方案：akshare的 /service/v5/stock/screener/screen 不受WAF限制
  - 数据：5272只股票讨论热度 + 个股新闻内容
- [x] weibo_stock.py — 改用金十数据微博舆情（stock_js_weibo_report + stock_news_em）
  - 原因：Sina Visitor Cookie系统需要JavaScript执行，返回432错误
  - 方案：金十数据中心提供预计算的微博舆情评分（50只热门股票）
  - 数据：多时间维度舆情评分（2h/6h/12h/1d/1w）+ 个股新闻
- [x] shizifengyun.py — 域名更新为wogoo.com + 改用财新新闻
  - 原因：shizifengyun.com已迁移至wogoo.com，SPA框架API需认证
  - 方案：akshare stock_news_main_cx（财新网100条新闻）+ stock_news_em
  - 数据：财新专业财经新闻 + 东方财富个股新闻
- [x] 集成测试：4/4数据源全部通过，155条帖子/3只股票
- **注意**: 需要同步到腾讯云服务器
...
- 雪球/微博/市值风云 已改用akshare接口，4/4数据源全部正常工作

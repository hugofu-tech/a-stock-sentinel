...
def get_market_overview():
    ...
    try:
        ...
        for _, row in df.iterrows():
            name = row.get('名称', '')
            if '上证' in name and '指数' in name and '能源' not in name and '等权' not in name:
                indices['sh'] = {'name': '上证指数', 'value': float(row.get('最新价', 0)), 'change': float(row.get('涨跌幅', 0))}
            elif '深证' in name and '成指' in name:
                indices['sz'] = {'name': '深证成指', 'value': float(row.get('最新价', 0)), 'change': float(row.get('涨跌幅', 0))}
            elif '创业板' in name and '指' in name:
                indices['cy'] = {'name': '创业板指', 'value': float(row.get('最新价', 0)), 'change': float(row.get('涨跌幅', 0))}
        return indices
    except Exception as e:
        print(f"获取市场概况失败: {e}")
        return {}
...
def get_sector_heat():
    ...
    try:
        ...
        for _, row in df.head(5).iterrows():
            sectors.append({'name': row['板块名称'], 'change': float(row['涨跌幅'])})
        return sectors
    except Exception as e:
        print(f"获取板块热度失败: {e}")
        return []
...
def main():
    ...
    if success:
        print("\n✅ 完成! 飞书消息已发送")
        # 保存报告
        filename = f'/Users/hugo/.openclaw/workspace/a_stock_sentinel/report_{datetime.now().strftime("%Y%m%d")}.md'
